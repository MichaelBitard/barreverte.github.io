...
# [nomadconf.eu](http://nomadconf.eu)

<img class="left" alt="La non-conférence NomadConf devrait traverser en train des zones montagneuses" src="/images/train_small.jpg"/>Ma plus grande découverte lors de l'open space du premier jour est [NomadConf](http://nomadconf.eu), une non-conférence sur le travail nomade : le vivre et l'expérimenter en continuant son travail depuis des trains qui traversent l'Europe.
...
- Trouver comment obtenir un badge est tellement difficile que la page [wiki](https://github.com/betagouv/beta.gouv.fr/wiki/Bienvenue#badge) de l'incubateur a été utilisée par un nouvel arrivant d'une autre entité qui était tombé dessus par hasard. Une fois éprouvée, la procédure a été automatisée dans un [robot](https://github.com/betagouv/betaGouvBot/blob/master/lib/betagouvbot/badge_request.rb) sur slack (il s'agit juste d'un [mail](https://github.com/betagouv/betaGouvBot/blob/master/data/mail_badge.md) a envoyer à la bonne personne avec les bonnes informations)
